...
const BASE = "https://www.prisma.io/docs";
const START = `${BASE}/orm`;
...
export async function crawlPrismaDocs() {
  const urls = await collectPrismaDocLinks();
  ...
  await Promise.all(
    urls.map(url =>
      limit(async () => {
        try {
          const doc = await parsePrismaDocToMd(url);
          results.push(doc);
          console.log(`✔ ${url}`);
        } catch (e) {
          console.error(`✖ ${url}`, e);
...
export async function collectPrismaDocLinks(): Promise<string[]> {
  ...
  $("a[href]").each((_, el) => {
    ...
    const url = new URL(href, "https://www.prisma.io").toString();
    links.add(url);
  });
...
export async function parsePrismaDocToMd(url: string): Promise<ParsedDoc> {
  ...
  const title =
    ...
    $("title").text().replace(" | Prisma Docs", "").trim();

  // чистка SPA-навигации
